...
impl EVMCodeGen {
    ...
    fn op_add(&mut self)  { self.code.push(0x01); }
    fn op_mul(&mut self)  { self.code.push(0x02); }
    fn op_sub(&mut self)  { self.code.push(0x03); }
    fn op_div(&mut self)  { self.code.push(0x04); }
    fn op_sdiv(&mut self) { self.code.push(0x05); }
    fn op_mod(&mut self)  { self.code.push(0x06); }
    fn op_smod(&mut self)  { self.code.push(0x07); }
    fn op_addmod(&mut self)  { self.code.push(0x08); }
    fn op_mulmod(&mut self)  { self.code.push(0x09); }
    fn op_exp(&mut self)  { self.code.push(0x0a); }
    fn op_signextend(&mut self)  { self.code.push(0x0b); }

    fn op_lt(&mut self)   { self.code.push(0x10); }
    fn op_gt(&mut self)   { self.code.push(0x11); }
    fn op_slt(&mut self)  { self.code.push(0x12); }
    fn op_sgt(&mut self)  { self.code.push(0x13); }
    fn op_eq(&mut self)   { self.code.push(0x14); }
    fn op_iszero(&mut self)  { self.code.push(0x15); }
    fn op_and(&mut self)  { self.code.push(0x16); }
    fn op_or(&mut self)  { self.code.push(0x17); }
    fn op_xor(&mut self)  { self.code.push(0x18); }
    fn op_not(&mut self)  { self.code.push(0x19); }
    fn op_byte(&mut self)  { self.code.push(0x1a); }

    fn op_keccak(&mut self) { self.code.push(0x20); }
    fn op_sha3(&mut self) { self.code.push(0x20); }   // alias

    fn op_address(&mut self)  { self.code.push(0x30); }
    fn op_balance(&mut self)  { self.code.push(0x31); }
    fn op_origin(&mut self)  { self.code.push(0x32); }
    fn op_caller(&mut self)  { self.code.push(0x33); }
    fn op_callvalue(&mut self)  { self.code.push(0x34); }
    fn op_calldataload(&mut self)  { self.code.push(0x35); }
    fn op_calldatasize(&mut self)  { self.code.push(0x36); }
    fn op_calldatacopy(&mut self)  { self.code.push(0x37); }
    fn op_codesize(&mut self)  { self.code.push(0x38); }
    fn op_codecopy(&mut self)  { self.code.push(0x39); }
    fn op_gasprice(&mut self)  { self.code.push(0x3a); }
    fn op_extcodesize(&mut self)  { self.code.push(0x3b); }
    fn op_extcodecopy(&mut self)  { self.code.push(0x3c); }
    fn op_returndatasize(&mut self)  { self.code.push(0x3d); }
    fn op_returndatacopy(&mut self)  { self.code.push(0x3e); }

    fn op_blockhash(&mut self)  { self.code.push(0x40); }
    fn op_coinbase(&mut self)  { self.code.push(0x41); }
    fn op_timestamp(&mut self)  { self.code.push(0x42); }
    fn op_number(&mut self)  { self.code.push(0x43); }
    fn op_difficulty(&mut self)  { self.code.push(0x44); }
    fn op_gaslimit(&mut self)  { self.code.push(0x45); }

    fn op_pop(&mut self)  { self.code.push(0x50); }
    fn op_mload(&mut self)  { self.code.push(0x51); }
    fn op_mstore(&mut self)  { self.code.push(0x52); }
    fn op_mstore8(&mut self)  { self.code.push(0x53); }
    fn op_sload(&mut self)  { self.code.push(0x54); }
    fn op_sstore(&mut self)  { self.code.push(0x55); }

    fn op_jump(&mut self, label: &str)  {
        let offset = self.ref_label(label,1);
        self.op_push(&offset);
        self.code.push(0x56);
    }

    fn op_jumpi(&mut self, label: &str)  {
        let offset = self.ref_label(label,1);
        self.op_push(&offset);
        self.code.push(0x57);
    }

    fn op_pc(&mut self)  { self.code.push(0x58); }
    fn op_msize(&mut self)  { self.code.push(0x59); }
    fn op_gas(&mut self)  { self.code.push(0x5a); }

    fn op_jumpdest(&mut self, name: &str)  {
    ...
    fn op_beginsub(&mut self, name: &str)  {
        if self.labels.contains_key(name) {
            panic!("Label already defined");
        }
        self.labels.insert(name.to_string(),self.code.len());
    ...
    fn op_jumpsub(&mut self, label: &str)  {
        let offset = self.ref_label(label,1);
        self.op_push(&offset);
        self.code.push(0xb3);
    }
    ...
    fn op_push(&mut self, data: &[u8]) { // push n-bytes item into the stack
    ...
    fn op_log0(&mut self)  { self.code.push(0xa0); }
    fn op_log1(&mut self)  { self.code.push(0xa1); }
    fn op_log2(&mut self)  { self.code.push(0xa2); }
    fn op_log3(&mut self)  { self.code.push(0xa3); }
    fn op_log4(&mut self)  { self.code.push(0xa4); }

    fn op_create(&mut self)  { self.code.push(0xf0); }
    fn op_call(&mut self)  { self.code.push(0xf1); }
    fn op_callcode(&mut self)  { self.code.push(0xf2); }
    fn op_return(&mut self)  { self.code.push(0xf3); }
    fn op_delegatecall(&mut self)  { self.code.push(0xf4); }

    fn op_staticcall(&mut self)  { self.code.push(0xfa); }
    fn op_revert(&mut self)  { self.code.push(0xfd); }
    fn op_invalid(&mut self)  { self.code.push(0xfe); }
    fn op_selfdestruct(&mut self)  { self.code.push(0xff); }
    ...
    fn gen_code(&self) -> Result<&Vec<u8>> {
       if self.pending_labels.is_empty() {
            ...
            Err(Error::msg(format!("pending_labels {:?}",list)))
        }
    }

    pub fn _gen_tx_code(&self) -> Result<Vec<u8>> {
    ...
    pub fn asm(code : &str) -> Result<Vec<u8>>{
        // format is
        //    [<LABEL>:]<NEMONIC> [<DATA>] # comments
        //    [<LABEL>::] # comments
        //    .data_segment -> .main is the main data segment

        let mut segments : HashMap<String, EVMCodeGen> = HashMap::new();
        let mut current_segment = EVMCodeGen::new();
        ...
        for original_line in code.split("\n") {
            ...
            if line.starts_with(".") {
                let mut new_segment = EVMCodeGen::new();
            ...
            match (op.as_str(),extra) {
                ...
                ("JUMP",Some(label)) => current_segment.op_jump(&label),
                ("JUMPI",Some(label)) => current_segment.op_jumpi(&label),
                ("PC",None) => current_segment.op_pc(),
                ("MSIZE",None) => current_segment.op_msize(),
                ("GAS",None) => current_segment.op_gas(),
                ("JUMPDEST",Some(label)) => current_segment.op_jumpdest(&label),
                ("BEGINSUB",Some(label)) => current_segment.op_beginsub(&label),
                ("JUMPSUB",Some(label)) => current_segment.op_jumpsub(&label),
                ("RETURNSUB",None) => current_segment.op_returnsub(),
                ("PUSH",Some(data)) => {
                    if data.starts_with("0x") {
                        current_segment.op_push(&hex::decode(&data[2..]).unwrap());
                    } else {
                        if data.ends_with(".len") {
                            let label = &data[..data.len()-".len".len()];
                            let len = segments[label].gen_code()?.len();
                            if len > 255 {
                                return Err(Error::msg(format!("Segment too big {}",len))); 
                            }
                            current_segment.op_push(&[len as u8]);
                        } else if data.ends_with(".offset") {
                            let label = &data[..data.len()-".offset".len()];
                            current_segment.push_data_segment(label);
                        } else {
                            return Err(Error::msg(format!("Unable to process line {}",original_line))); 
                        }
                    }
                },
                ("DUP1", None) => current_segment.op_dup(1),
                ("DUP2", None) => current_segment.op_dup(2),
                ("DUP3", None) => current_segment.op_dup(3),
                ("DUP4", None) => current_segment.op_dup(4),
                ("DUP5", None) => current_segment.op_dup(5),
                ("DUP6", None) => current_segment.op_dup(6),
                ("DUP7", None) => current_segment.op_dup(7),
                ("DUP8", None) => current_segment.op_dup(8),
                ("DUP9", None) => current_segment.op_dup(9),
                ("DUP10", None) => current_segment.op_dup(10),
                ("DUP11", None) => current_segment.op_dup(11),
                ("DUP12", None) => current_segment.op_dup(12),
                ("DUP13", None) => current_segment.op_dup(13),
                ("DUP14", None) => current_segment.op_dup(14),
                ("DUP15", None) => current_segment.op_dup(15),
                ("DUP16", None) => current_segment.op_dup(16),
                ("SWAP", None) => current_segment.op_swap(1),
                ("SWAP2", None) => current_segment.op_swap(2),
                ("SWAP3", None) => current_segment.op_swap(3),
                ("SWAP4", None) => current_segment.op_swap(4),
                ("SWAP5", None) => current_segment.op_swap(5),
                ("SWAP6", None) => current_segment.op_swap(6),
                ("SWAP7", None) => current_segment.op_swap(7),
                ("SWAP8", None) => current_segment.op_swap(8),
                ("SWAP9", None) => current_segment.op_swap(9),
                ("SWAP10", None) => current_segment.op_swap(10),
                ("SWAP11", None) => current_segment.op_swap(11),
                ("SWAP12", None) => current_segment.op_swap(12),
                ("SWAP13", None) => current_segment.op_swap(13),
                ("SWAP14", None) => current_segment.op_swap(14),
                ("SWAP15", None) => current_segment.op_swap(15),
                ("SWAP16", None) => current_segment.op_swap(16),
                ...
                _ => return Err(Error::msg(format!("Unable to process line {}",original_line)))
            }
        }
        let mut new_segment = EVMCodeGen::new();
        std::mem::swap(&mut current_segment, &mut new_segment);
